"""`audiodsp_util`: one number, the same number, on every target.

A setting written in Python is held at the interpreter's `mp_float_t` width --
double here and on a desktop MicroPython, single on every board audiodsp ships
...
print one. `docs/correctness-standard.md` carries the rule; audiodsp#80 is where
...
from audiodsp_util import float32, float32_bits


#: Every inexact literal the six probes of audiodsp#80 hand to a node, plus the
