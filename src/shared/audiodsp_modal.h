...
//
// WHY IT IS PARAMETERISED BY DECAY TIME AND NOT Q. `audiodsp_filter_f32.c`
// clamps Q to AUDIODSP_FILTER_F32_MAX_Q = 60, and says why: an RBJ section's
...
// AUDIODSP_MODAL_MAX_DECAY is a number a reader can check against the music.
//
...
// AUDIODSP_MODAL_FLUSH, a mode that has finished ringing is *exactly* zero,
...
// audiodsp_filter_f32.c or audiodsp_dynamics.c: each runtime reaches its audio
// graph differently.
//
...
//: Frames one output block carries. The same 256 every other audiodsp node
//: uses, so a chain built out of several of them moves in one block size.
#define AUDIODSP_MODAL_FRAMES 256u
...
#define AUDIODSP_MODAL_MAX_MODES 64u

//: Any state word smaller than this is written as exact zero. Same threshold
//: and same reasoning as AUDIODSP_FILTER_F32_FLUSH: well above the float32
...
#define AUDIODSP_MODAL_FLUSH 1e-20f
...
#define AUDIODSP_MODAL_MIN_DECAY 0.001f
#define AUDIODSP_MODAL_MAX_DECAY 30.0f

//: The settable per-bank options, in the order the bindings' keyword tables
//: list them.
typedef enum {
    AUDIODSP_MODAL_OPT_MIX = 0,
    AUDIODSP_MODAL_OPT_GAIN,
} audiodsp_modal_option_t;
...
typedef struct {
    ...
    float decay;
    float gain;
} audiodsp_modal_mode_t;

//: Per-mode recursion coefficients, written by audiodsp_modal_config_finish()
//: from the mode table above.
typedef struct {
    float b0;       // gain * sin(w0) -- normalises the IR peak to `gain`
    float a1, a2;   // -2*r*cos(w0), r*r
} audiodsp_modal_coeff_t;
...
typedef struct {
    ...
    bool derived;
} audiodsp_modal_config_t;

//: Transposed direct form II, two memory words per mode per channel -- the
//: same form and for the same reason as audiodsp_filter_f32.c's biquad
//: (audiodsp#64): with the poles this close to the unit circle, direct form I
...
} audiodsp_modal_state_t;

void audiodsp_modal_config_init(audiodsp_modal_config_t *config,
    uint32_t sample_rate, uint32_t channel_count, uint32_t mode_count,
    audiodsp_modal_mode_t *modes, audiodsp_modal_coeff_t *coeffs);

// Applies immediately. Values are clamped here, once, rather than in each
// binding.
void audiodsp_modal_configure(audiodsp_modal_config_t *config,
    audiodsp_modal_option_t option, float value);
...
void audiodsp_modal_set_mode(audiodsp_modal_config_t *config, uint32_t index,
...
void audiodsp_modal_config_finish(audiodsp_modal_config_t *config);
...
uint32_t audiodsp_modal_state_floats(const audiodsp_modal_config_t *config);

void audiodsp_modal_state_init(audiodsp_modal_state_t *state,
    const audiodsp_modal_config_t *config, float *s1, float *s2,
    uint32_t words);
...
void audiodsp_modal_reset(audiodsp_modal_state_t *state);
...
// `ringing`.
bool audiodsp_modal_silent(const audiodsp_modal_state_t *state);

// Interleaved 16-bit frames in and out. `out` may alias `in`.
void audiodsp_modal_process_s16(const audiodsp_modal_config_t *config,
    audiodsp_modal_state_t *state, int16_t *out, const int16_t *in,
    uint32_t frames);
