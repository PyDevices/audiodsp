// Runtime-neutral signal splitter. See audiodsp_splitter.h for provenance.
...
void audiodsp_splitter_init(audiodsp_splitter_state_t *state,
    uint32_t tap_count) {
...
uint32_t audiodsp_splitter_write(audiodsp_splitter_state_t *state,
    ...
    if (count > AUDIODSP_SPLITTER_RING_FRAMES) {
        count = AUDIODSP_SPLITTER_RING_FRAMES;
    }
    const uint32_t taken = count;
    while (count-- != 0) {
        const uint32_t at =
            (state->write_pos % AUDIODSP_SPLITTER_RING_FRAMES) * 2u;
...
uint32_t audiodsp_splitter_take(audiodsp_splitter_state_t *state, uint32_t tap,
    ...
    const uint32_t start = state->read_pos[tap] % AUDIODSP_SPLITTER_RING_FRAMES;
    uint32_t run = AUDIODSP_SPLITTER_RING_FRAMES - start;
    if (run > available) {
        run = available;
    }
    if (run > AUDIODSP_SPLITTER_CHUNK_FRAMES) {
        run = AUDIODSP_SPLITTER_CHUNK_FRAMES;
    }
    *start_frame = start;
