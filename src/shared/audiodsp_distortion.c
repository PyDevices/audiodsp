...
int32_t audiodsp_distortion_sample(int32_t sample, double drive,
    ...
    if (mode == AUDIODSP_DISTORTION_LOFI) word &= word_mask;

    if (mode != AUDIODSP_DISTORTION_LOFI || soft_clip) {
        double value = word / 32768.0;
        switch (mode) {
            case AUDIODSP_DISTORTION_CLIP:
                value = pow(fabs(value), drive);
                if (word < 0) value *= -1.0;
                break;
            case AUDIODSP_DISTORTION_LOFI:
                break;
            case AUDIODSP_DISTORTION_OVERDRIVE: {
                value *= 0.686306;
            ...
            case AUDIODSP_DISTORTION_WAVESHAPE:
...
void audiodsp_distortion_process_s16(int16_t *output, const int16_t *input,
    ...
    if (mode == AUDIODSP_DISTORTION_CLIP) {
        drive = 1.0001 - drive;
    } else if (mode == AUDIODSP_DISTORTION_WAVESHAPE) {
        drive = 2.0 * drive / (1.0001 - drive);
    } else if (mode == AUDIODSP_DISTORTION_LOFI) {
        word_mask = 0xffffffffU ^ ((1U << (uint32_t)round(drive * 14.0)) - 1U);
    }
    for (size_t i = 0; i < count; i++) {
        output[i] = (int16_t)audiodsp_distortion_sample(input[i], drive,
