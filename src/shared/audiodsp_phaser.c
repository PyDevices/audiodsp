...
void audiodsp_phaser_process_s16(int16_t *samples, size_t sample_count,
    ...
    int16_t coefficient = (int16_t)((1.0 - frequency) /
        (1.0 + frequency) * 32767);
    audiodsp_phaser_process_s16_fixed(samples, sample_count, feedback_words,
        allpass_words, channel_count, stages, coefficient, feedback, mix);
}

void audiodsp_phaser_process_s16_fixed(int16_t *samples, size_t sample_count,
    int16_t *feedback_words, int16_t *allpass_words, uint8_t channel_count,
    ...
    for (size_t i = 0; i < sample_count; i++) {
        ...
        int32_t word = audiodsp_sat16(sample + audiodsp_sat16(
            (int32_t)feedback_words[channel] * feedback, 15), 0);
        for (uint8_t stage = 0; stage < stages; stage++) {
            size_t index = offset + stage;
            int32_t allpass = audiodsp_sat16(audiodsp_sat16(
                word * -coefficient, 15) + allpass_words[index], 0);
            allpass_words[index] = audiodsp_sat16(audiodsp_sat16(
                allpass * coefficient, 15) + word, 0);
            word = allpass;
        }
        feedback_words[channel] = (int16_t)word;
        samples[i] = audiodsp_mix_down_sample(
            sample + audiodsp_sat16(word * mix, 15), 2, -28000, 28000);
    }
}
