// The sample-and-hold kernel. See audioif_samplehold.h for why it exists and
// why the accumulator is a pair of integers rather than a finer fixed point.
//
// No `shared/audioif_fp_contract.h` here, deliberately: there is no float in
// this file at all. The contraction rule (audioif#79) is about a compiler
...
bool audioif_samplehold_ratio_ok(uint32_t num, uint32_t den) {
    ...
    return num <= AUDIOIF_SAMPLEHOLD_MAX_RATIO;
}
...
bool audioif_samplehold_config_set(audioif_samplehold_config_t *config,
    ...
    audioif_samplehold_config_init(config, num, den);
...
void audioif_samplehold_state_init(audioif_samplehold_state_t *state,
    const audioif_samplehold_config_t *config) {
    audioif_samplehold_reset(state, config);
}
