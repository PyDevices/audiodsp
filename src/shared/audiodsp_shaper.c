// Runtime-neutral oversampled table waveshaper. See audiodsp_shaper.h for what
// it is for and why none of it could be composed out of the palette.
//
// `float` working precision, deliberately, to match audiodsp_dynamics.c and
// audiodsp_feedback_delay.c: this runs per *oversampled* sample on parts
...
static const float audiodsp_shaper_hb_coefficients[2][AUDIODSP_SHAPER_HB_SECTIONS] = {
...
void audiodsp_shaper_set_oversample(audiodsp_shaper_config_t *config,
    uint32_t oversample) {
    uint32_t stages = 0;
    while (stages < AUDIODSP_SHAPER_MAX_STAGES && (2u << stages) <= oversample) {
        ++stages;
    }
...
void audiodsp_shaper_set_channel_count(audiodsp_shaper_config_t *config,
    uint32_t channel_count) {
    config->channel_count = channel_count == 1u ? 1u : 2u;
}

void audiodsp_shaper_set_curve(audiodsp_shaper_config_t *config,
...
void audiodsp_shaper_config_init(audiodsp_shaper_config_t *config,
    uint32_t sample_rate, uint32_t oversample) {
    config->sample_rate = sample_rate ? sample_rate : 48000;
    config->channel_count = 2;
    audiodsp_shaper_set_oversample(config, oversample);
    config->curve = NULL;
    ...
    audiodsp_shaper_config_finish(config);
}

void audiodsp_shaper_configure(audiodsp_shaper_config_t *config,
    audiodsp_shaper_option_t option, float value) {
    switch (option) {
        case AUDIODSP_SHAPER_OPT_PRE_GAIN:
            ...
            break;
        case AUDIODSP_SHAPER_OPT_BIAS:
            config->bias = clampf(value, -1.0f, 1.0f);
            break;
        case AUDIODSP_SHAPER_OPT_POST_GAIN:
            config->post_gain = clampf(value, -1000.0f, 1000.0f);
            break;
        case AUDIODSP_SHAPER_OPT_MIX:
            config->mix = clampf(value, 0.0f, 1.0f);
            break;
        case AUDIODSP_SHAPER_OPT_HYSTERESIS:
            config->hysteresis = clampf(value, 0.0f, 1.0f);
            break;
        case AUDIODSP_SHAPER_OPT_HYSTERESIS_WIDTH:
            ...
            break;
        case AUDIODSP_SHAPER_OPT_HYSTERESIS_BIAS:
            config->hysteresis_bias = clampf(value, -1.0f, 1.0f);
            break;
    }
}

void audiodsp_shaper_config_finish(audiodsp_shaper_config_t *config) {
...
void audiodsp_shaper_state_init(audiodsp_shaper_state_t *state) {
    memset(state, 0, sizeof(*state));
}

void audiodsp_shaper_reset(audiodsp_shaper_state_t *state) {
    audiodsp_shaper_state_init(state);
}
...
static float allpass_branch(audiodsp_shaper_halfband_t *halfband,
    uint32_t branch, float value) {
    for (uint32_t section = 0; section < AUDIODSP_SHAPER_HB_SECTIONS;
         ++section) {
        const float a = audiodsp_shaper_hb_coefficients[branch][section];
...
static void halfband_up(audiodsp_shaper_halfband_t *halfband, float value,
    float *out) {
...
static float halfband_down(audiodsp_shaper_halfband_t *halfband, float even,
    float odd) {
...
static float curve_lookup(const audiodsp_shaper_config_t *config, float value) {
...
static float shape_sample(const audiodsp_shaper_config_t *config,
    audiodsp_shaper_state_t *state, uint32_t channel, float value) {
...
void audiodsp_shaper_process_s16(const audiodsp_shaper_config_t *config,
    audiodsp_shaper_state_t *state, int16_t *out, const int16_t *in,
    uint32_t frames) {
    ...
    for (uint32_t frame = 0; frame < frames; ++frame) {
        for (uint32_t channel = 0; channel < channels; ++channel) {
            ...
            float oversampled[AUDIODSP_SHAPER_MAX_OVERSAMPLE];
            float expanded[AUDIODSP_SHAPER_MAX_OVERSAMPLE];
