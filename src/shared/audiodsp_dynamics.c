// Runtime-neutral dynamics DSP. See audiodsp_dynamics.h for provenance.
// SPDX-License-Identifier: MIT

// No fused multiply-add here. audiodsp#66 proved contraction was the whole
// of the P4-vs-S3 split on this file's transient attack path and turned it
// off inline; audiodsp#79 generalised it to the rest of the kernel, so the
// mechanism, the two spellings that do not work and the measurements now
// live in one place. Same pragma, same codegen -- checked, not assumed.
...
float audiodsp_dynamics_ms_to_coef(float ms, float sample_rate) {
    if (ms <= 0.0f) {
        return 1.0f;
    }
...
float audiodsp_dynamics_db_to_gain(float db) {
    return expf(db * 0.115129254649702f);
}

float audiodsp_dynamics_gain_to_db(float gain) {
    return logf(gain < 1e-6f ? 1e-6f : gain) * 8.68588963806504f;
}

void audiodsp_dynamics_config_init(audiodsp_dynamics_config_t *config, int mode,
    ...
    config->true_peak = AUDIODSP_DYNAMICS_TP_OFF;
    ...
    config->detector = AUDIODSP_DYNAMICS_DETECT_PEAK;
...
void audiodsp_dynamics_config_finish(audiodsp_dynamics_config_t *config) {
    if (config->attack_coef == 0.0f) {
        config->attack_coef = audiodsp_dynamics_ms_to_coef(10.0f,
            (float)config->sample_rate);
    }
    if (config->release_coef == 0.0f) {
        config->release_coef = audiodsp_dynamics_ms_to_coef(120.0f,
            (float)config->sample_rate);
    }
    ...
    if (config->rms_coef == 0.0f) {
        config->rms_coef = audiodsp_dynamics_ms_to_coef(10.0f,
            (float)config->sample_rate);
    }
}

void audiodsp_dynamics_configure(audiodsp_dynamics_config_t *config,
    audiodsp_dynamics_option_t option, float value) {
    switch (option) {
        case AUDIODSP_DYNAMICS_OPT_THRESHOLD_DB:
            config->threshold_db = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_RATIO:
            config->ratio = value < 1.0f ? 1.0f : value;
            break;
        case AUDIODSP_DYNAMICS_OPT_KNEE_DB:
            config->knee_db = value < 0.0f ? 0.0f : value;
            break;
        case AUDIODSP_DYNAMICS_OPT_MAKEUP_DB:
            config->makeup_gain = audiodsp_dynamics_db_to_gain(value);
            break;
        case AUDIODSP_DYNAMICS_OPT_ATTACK_MS:
            config->attack_coef = audiodsp_dynamics_ms_to_coef(value,
                (float)config->sample_rate);
            break;
        case AUDIODSP_DYNAMICS_OPT_RELEASE_MS:
            config->release_coef = audiodsp_dynamics_ms_to_coef(value,
                (float)config->sample_rate);
            break;
        case AUDIODSP_DYNAMICS_OPT_ATTACK_GAIN_DB:
            config->attack_gain_db = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_SUSTAIN_GAIN_DB:
            config->sustain_gain_db = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_SIDECHAIN_HZ:
            config->sidechain_coef = value <= 0.0f ? 0.0f
                : 1.0f - expf(-6.283185307f * value /
                              (float)config->sample_rate);
            break;
        case AUDIODSP_DYNAMICS_OPT_LOOKAHEAD_MS: {
            float ms = value;
            if (ms < 0.0f) {
                ms = 0.0f;
            } else if (ms > AUDIODSP_DYNAMICS_MAX_LOOKAHEAD_MS) {
                ms = AUDIODSP_DYNAMICS_MAX_LOOKAHEAD_MS;
            }
            config->lookahead_frames =
                (uint32_t)(ms * (float)config->sample_rate / 1000.0f);
            break;
        }
        case AUDIODSP_DYNAMICS_OPT_TRUE_PEAK:
            ...
            config->true_peak = value <= 0.0f ? AUDIODSP_DYNAMICS_TP_OFF
                : (value < 2.0f ? AUDIODSP_DYNAMICS_TP_HALF_BAND
                                : AUDIODSP_DYNAMICS_TP_OVERSAMPLED);
            break;
        case AUDIODSP_DYNAMICS_OPT_TRANSIENT_FAST_ATTACK_MS:
            config->transient_fast_attack_ms = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_TRANSIENT_FAST_RELEASE_MS:
            config->transient_fast_release_ms = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_TRANSIENT_SLOW_ATTACK_MS:
            config->transient_slow_attack_ms = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_TRANSIENT_SLOW_RELEASE_MS:
            config->transient_slow_release_ms = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_DETECTOR:
            config->detector = value >= 1.0f ? AUDIODSP_DYNAMICS_DETECT_RMS
                                             : AUDIODSP_DYNAMICS_DETECT_PEAK;
            break;
        case AUDIODSP_DYNAMICS_OPT_RMS_MS:
            config->rms_coef = audiodsp_dynamics_ms_to_coef(value,
                (float)config->sample_rate);
            break;
        case AUDIODSP_DYNAMICS_OPT_GAIN_SMOOTH_MS:
            ...
            config->gain_smooth_coef = audiodsp_dynamics_ms_to_coef(value,
                (float)config->sample_rate);
            break;
        case AUDIODSP_DYNAMICS_OPT_FEEDBACK_DETECTOR:
            config->feedback_detector = value != 0.0f;
            break;
        case AUDIODSP_DYNAMICS_OPT_FEEDBACK_GAIN_CORRECTED:
            config->feedback_gain_corrected = value != 0.0f;
            break;
        case AUDIODSP_DYNAMICS_OPT_SIDECHAIN_LP_HZ:
            config->sidechain_lp_coef = value <= 0.0f ? 0.0f
                : 1.0f - expf(-6.283185307f * value /
                              (float)config->sample_rate);
            break;
        case AUDIODSP_DYNAMICS_OPT_SIDECHAIN_POLES:
            config->sidechain_poles = value >= 2.0f ? 2u : 1u;
            break;
        case AUDIODSP_DYNAMICS_OPT_KEY_LISTEN:
            config->key_listen = value != 0.0f;
            break;
        case AUDIODSP_DYNAMICS_OPT_DEPTH_DB:
            config->depth_db = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_HOLD_MS:
            config->hold_frames = value <= 0.0f ? 0u
                : (uint32_t)(value * (float)config->sample_rate / 1000.0f);
            break;
        case AUDIODSP_DYNAMICS_OPT_HYSTERESIS_DB:
            // A hysteresis is a width, so its sign carries no information.
            config->hysteresis_db = value < 0.0f ? -value : value;
            break;
        case AUDIODSP_DYNAMICS_OPT_RELATIVE_THRESHOLD:
            config->relative_threshold = value != 0.0f;
            break;
        case AUDIODSP_DYNAMICS_OPT_PROGRAM_ATTACK:
            config->program_attack = value != 0.0f;
            break;
        case AUDIODSP_DYNAMICS_OPT_TRANSIENT_DUAL:
            config->transient_dual = value != 0.0f;
            break;
        case AUDIODSP_DYNAMICS_OPT_SUSTAIN_FAST_ATTACK_MS:
            config->sustain_fast_attack_ms = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_SUSTAIN_FAST_RELEASE_MS:
            config->sustain_fast_release_ms = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_SUSTAIN_SLOW_ATTACK_MS:
            config->sustain_slow_attack_ms = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_SUSTAIN_SLOW_RELEASE_MS:
            config->sustain_slow_release_ms = value;
            break;
        case AUDIODSP_DYNAMICS_OPT_SLOW_HOLD_MS:
            config->slow_hold_frames = value <= 0.0f ? 0u
                : (uint32_t)(value * (float)config->sample_rate / 1000.0f);
            break;
        case AUDIODSP_DYNAMICS_OPT_COUNT:
        default:
            break;
    }
}

uint32_t audiodsp_dynamics_lookahead_frames(
    const audiodsp_dynamics_config_t *config) {
    return config->lookahead_frames;
}

void audiodsp_dynamics_set_lookahead(audiodsp_dynamics_state_t *state,
...
// not mean what it says, and that is exactly what audiodsp#56 was -- measured,
// a fresh node under a quiet tone gated it to 0 LSB and the same node after a
// loud pass and a reset passed it at 32.
void audiodsp_dynamics_clear_detector(audiodsp_dynamics_state_t *state) {
    ...
    audiodsp_dynamics_clear_extras(state);
}

void audiodsp_dynamics_state_init(audiodsp_dynamics_state_t *state) {
    state->channel_count = 2;
    state->lookahead = NULL;
    state->lookahead_capacity = 0;
    state->lookahead_write = 0;
    audiodsp_dynamics_clear_detector(state);
}
...
// and all four are cleared by audiodsp_dynamics_clear_detector above, which is
// the single door both state_init and reset go through.
void audiodsp_dynamics_clear_extras(audiodsp_dynamics_state_t *state) {
    ...
    state->gate_stage = AUDIODSP_DYNAMICS_GATE_CLOSED;
    state->gate_gain = 0.0f;
    state->hold_left = 0;
    memset(state->tp_history, 0, sizeof(state->tp_history));
}

void audiodsp_dynamics_set_channel_count(
    audiodsp_dynamics_config_t *config, audiodsp_dynamics_state_t *state,
...
void audiodsp_dynamics_reset(audiodsp_dynamics_state_t *state) {
...
static const float dynamics_tp_taps[AUDIODSP_DYNAMICS_TP_PHASES]
                                   [AUDIODSP_DYNAMICS_TP_TAPS] = {
    {
...
static float oversampled_peak(const float *history) {
    float peak = 0.0f;
    for (uint32_t phase = 0; phase < AUDIODSP_DYNAMICS_TP_PHASES; ++phase) {
        float sum = 0.0f;
        for (uint32_t tap = 0; tap < AUDIODSP_DYNAMICS_TP_TAPS; ++tap) {
...
static float dynamics_gain_db(const audiodsp_dynamics_config_t *config,
    float env_db) {
    const float over = env_db - config->threshold_db;
    switch (config->mode) {
        case AUDIODSP_DYNAMICS_LIMIT:
            return over > 0.0f ? -over : 0.0f;
        case AUDIODSP_DYNAMICS_EXPAND: {
            if (over >= 0.0f) {
                return 0.0f;
            }
        ...
        case AUDIODSP_DYNAMICS_COMPRESS:
        default: {
...
void audiodsp_dynamics_process_s16(const audiodsp_dynamics_config_t *config,
    audiodsp_dynamics_state_t *state, int16_t *output, const int16_t *input,
    uint32_t frames) {
    audiodsp_dynamics_process_s16_key(config, state, output, input, NULL,
        frames);
}

void audiodsp_dynamics_process_s16_key(const audiodsp_dynamics_config_t *config,
    audiodsp_dynamics_state_t *state, int16_t *output, const int16_t *input,
    ...
    const bool gate_machine = config->hold_frames != 0u &&
        config->mode == AUDIODSP_DYNAMICS_GATE;
    const float open_level = gate_machine
        ? audiodsp_dynamics_db_to_gain(config->threshold_db) : 0.0f;
    const float close_level = gate_machine
        ? audiodsp_dynamics_db_to_gain(config->threshold_db -
                                      config->hysteresis_db) : 0.0f;
    const float floor_gain = gate_machine
        ? audiodsp_dynamics_db_to_gain(config->depth_db > 0.0f
    ...
    while (frames-- != 0) {
        ...
        float level = 0.0f;
        if (config->detector == AUDIODSP_DYNAMICS_DETECT_RMS) {
        ...
        if (config->true_peak == AUDIODSP_DYNAMICS_TP_HALF_BAND) {
            for (uint32_t channel = 0; channel < channels; ++channel) {
        ...
        } else if (config->true_peak == AUDIODSP_DYNAMICS_TP_OVERSAMPLED) {
            for (uint32_t channel = 0; channel < channels; ++channel) {
                for (uint32_t tap = 1; tap < AUDIODSP_DYNAMICS_TP_TAPS; ++tap) {
                    state->tp_history[channel][tap - 1] =
                        state->tp_history[channel][tap];
                }
                state->tp_history[channel][AUDIODSP_DYNAMICS_TP_TAPS - 1] =
                    detector[channel];
                const float between =
        ...
        float gain;
        if (config->mode == AUDIODSP_DYNAMICS_TRANSIENT) {
            ...
            // rather than an arbitrary guard -- audiodsp#57 asks for it to be
            ...
            // that sweep.
            const float diff =
                audiodsp_dynamics_gain_to_db(state->fast_env + 1e-5f) -
                audiodsp_dynamics_gain_to_db(state->slow_env + 1e-5f);
            ...
            if (config->transient_dual) {
                ...
                const float diff2 =
                    audiodsp_dynamics_gain_to_db(state->fast_env2 + 1e-5f) -
                    audiodsp_dynamics_gain_to_db(state->slow_env2 + 1e-5f);
            ...
            gain = audiodsp_dynamics_db_to_gain(gain_db) * config->makeup_gain;
            ...
            if (level >= open_level &&
                state->gate_stage != AUDIODSP_DYNAMICS_GATE_HOLD) {
                state->gate_stage = AUDIODSP_DYNAMICS_GATE_ATTACK;
            }
            switch (state->gate_stage) {
                case AUDIODSP_DYNAMICS_GATE_ATTACK:
                    ...
                    break;
                case AUDIODSP_DYNAMICS_GATE_HOLD:
                    state->gate_gain = 1.0f;
                    ...
                    if (level >= close_level) {
                        ...
                        state->gate_stage = AUDIODSP_DYNAMICS_GATE_DECAY;
                    }
                    break;
                case AUDIODSP_DYNAMICS_GATE_DECAY:
                    ...
                    break;
                case AUDIODSP_DYNAMICS_GATE_CLOSED:
                default:
                    state->gate_gain = floor_gain;
                    break;
            }
            state->gain_reduction_db =
                audiodsp_dynamics_gain_to_db(state->gate_gain);
            ...
            float env_db =
                audiodsp_dynamics_gain_to_db(state->envelope + 1e-6f);
            if (config->relative_threshold) {
                ...
                env_db -= audiodsp_dynamics_gain_to_db(state->full_env + 1e-6f);
            }
