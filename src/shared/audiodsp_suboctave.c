// Runtime-neutral octave divider. See audiodsp_suboctave.h for provenance.
...
#define AUDIODSP_SUBOCTAVE_DEFAULT_HOLD_MS 1.0f
...
#define AUDIODSP_SUBOCTAVE_DEFAULT_THRESHOLD 328

void audiodsp_suboctave_config_init(audiodsp_suboctave_config_t *config,
    uint32_t sample_rate) {
    config->sample_rate = sample_rate < 1u ? 1u : sample_rate;
    config->channel_count = 2;
    config->order = 1;
    config->mix = 32768;
    config->threshold = AUDIODSP_SUBOCTAVE_DEFAULT_THRESHOLD;
    config->hold_ms = AUDIODSP_SUBOCTAVE_DEFAULT_HOLD_MS;
    config->hold_frames = 0;
    audiodsp_suboctave_config_finish(config);
}

void audiodsp_suboctave_configure(audiodsp_suboctave_config_t *config,
    audiodsp_suboctave_option_t option, float value) {
    switch (option) {
        case AUDIODSP_SUBOCTAVE_OPT_ORDER:
            config->order = value >= 2.0f ? 2u : 1u;
            break;
        case AUDIODSP_SUBOCTAVE_OPT_MIX:
            if (value <= 0.0f) {
                config->mix = 0;
            } else if (value >= 1.0f) {
                config->mix = 32768;
                ...
                // the same trade audiodsp_multiply.c makes.
                config->mix = (int32_t)(value * 32768.0f + 0.5f);
            }
            break;
        case AUDIODSP_SUBOCTAVE_OPT_THRESHOLD:
            if (value <= 0.0f) {
            ...
            break;
        case AUDIODSP_SUBOCTAVE_OPT_HOLD_MS:
...
void audiodsp_suboctave_config_finish(audiodsp_suboctave_config_t *config) {
...
void audiodsp_suboctave_set_channel_count(audiodsp_suboctave_config_t *config,
    uint32_t channel_count) {
    config->channel_count = channel_count == 1u ? 1u : 2u;
}

void audiodsp_suboctave_state_init(audiodsp_suboctave_state_t *state) {
    audiodsp_suboctave_reset(state);
}

void audiodsp_suboctave_reset(audiodsp_suboctave_state_t *state) {
...
void audiodsp_suboctave_process_s16(const audiodsp_suboctave_config_t *config,
    audiodsp_suboctave_state_t *state, int16_t *out, const int16_t *in,
    uint32_t frames) {
    ...
    for (uint32_t frame = 0; frame < frames; ++frame) {
        ...
        for (uint32_t channel = 0; channel < channels; ++channel) {
            ...
            // one multiply fewer than audiodsp_multiply.c's blend, and the one
