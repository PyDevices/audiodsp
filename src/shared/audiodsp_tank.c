// Runtime-neutral reverberation tank. See audiodsp_tank.h for what the palette
...
// audiodsp_feedback_delay.c: this runs per sample on parts without an FPU at
...
#define AUDIODSP_TANK_PI 3.14159265358979323846f
...
#define AUDIODSP_TANK_REFERENCE_RATE 29761.0f
...
#define AUDIODSP_TANK_INPUT_DIFFUSION_2 0.83333333f
#define AUDIODSP_TANK_DECAY_DIFFUSION_1 0.93333333f
#define AUDIODSP_TANK_DECAY_DIFFUSION_2 0.66666667f
...
//: then half B.
static const uint16_t audiodsp_tank_reference_lines[AUDIODSP_TANK_LINES] = {
    142u, 107u, 379u, 277u,
...
static const struct {
    ...
    uint16_t offset;
    float gain;
} audiodsp_tank_reference_taps[AUDIODSP_TANK_DEFAULT_TAPS] = {
    { 0u,  9u,  266u,  0.6f },
    { 0u,  9u, 2974u,  0.6f },
    { 0u, 10u, 1913u, -0.6f },
    { 0u, 11u, 1996u,  0.6f },
...
// audiodsp_feedback_delay.c's, and deliberately the same shape.
static float one_pole_coefficient(float hz, uint32_t sample_rate) {
    if (hz <= 0.0f || sample_rate == 0) {
        return 0.0f;
    }
    float coefficient =
        1.0f - expf(-2.0f * AUDIODSP_TANK_PI * hz / (float)sample_rate);
...
void audiodsp_tank_default_delays(uint32_t sample_rate, uint32_t *delays) {
    for (uint32_t line = 0; line < AUDIODSP_TANK_LINES; ++line) {
        uint32_t frames =
            scale_to_rate(audiodsp_tank_reference_lines[line], sample_rate);
        if (frames < AUDIODSP_TANK_MIN_LINE) {
            frames = AUDIODSP_TANK_MIN_LINE;
        }
        delays[line] = frames;
    }
}
...
uint32_t audiodsp_tank_default_taps(uint32_t sample_rate, float *taps) {
    uint32_t lines[AUDIODSP_TANK_LINES];
    audiodsp_tank_default_delays(sample_rate, lines);
    for (uint32_t tap = 0; tap < AUDIODSP_TANK_DEFAULT_TAPS; ++tap) {
        const uint32_t line = audiodsp_tank_reference_taps[tap].line;
        uint32_t offset =
            scale_to_rate(audiodsp_tank_reference_taps[tap].offset, sample_rate);
        if (offset >= lines[line]) {
            offset = lines[line] - 1u;
        }
        taps[tap * 4u + 0u] = (float)audiodsp_tank_reference_taps[tap].channel;
        taps[tap * 4u + 1u] = (float)line;
        taps[tap * 4u + 2u] = (float)offset;
        taps[tap * 4u + 3u] = audiodsp_tank_reference_taps[tap].gain;
    }
    return AUDIODSP_TANK_DEFAULT_TAPS;
}

audiodsp_tank_status_t audiodsp_tank_set_delays(audiodsp_tank_config_t *config,
    const uint32_t *frames, uint32_t count) {
    if (count != AUDIODSP_TANK_LINES) {
        return AUDIODSP_TANK_ERR_COUNT;
    }
    uint32_t total = 0;
    for (uint32_t line = 0; line < AUDIODSP_TANK_LINES; ++line) {
        if (frames[line] < AUDIODSP_TANK_MIN_LINE) {
            return AUDIODSP_TANK_ERR_LENGTH;
        }
        ...
        if (frames[line] > (1u << 22)) {
            return AUDIODSP_TANK_ERR_TOTAL;
        }
        total += frames[line];
        if (total > (1u << 23)) {
            return AUDIODSP_TANK_ERR_TOTAL;
...
audiodsp_tank_status_t audiodsp_tank_set_taps(audiodsp_tank_config_t *config,
    ...
    if (count % 4u != 0u || count > AUDIODSP_TANK_MAX_TAPS * 4u) {
        return AUDIODSP_TANK_ERR_COUNT;
    }
    ...
    for (uint32_t tap = 0; tap < taps; ++tap) {
        ...
        if (channel < 0.0f || channel > 1.0f) {
            return AUDIODSP_TANK_ERR_CHANNEL;
        }
        if (line < 0.0f || line >= (float)AUDIODSP_TANK_LINES) {
            return AUDIODSP_TANK_ERR_LINE;
        }
        if (offset < 0.0f ||
            offset >= (float)config->line_frames[(uint32_t)line]) {
            return AUDIODSP_TANK_ERR_OFFSET;
        }
    }
    for (uint32_t tap = 0; tap < taps; ++tap) {
...
void audiodsp_tank_config_init(audiodsp_tank_config_t *config,
    ...
    audiodsp_tank_default_delays(config->sample_rate, config->line_frames);
    float taps[AUDIODSP_TANK_DEFAULT_TAPS * 4u];
    const uint32_t count =
        audiodsp_tank_default_taps(config->sample_rate, taps);
    (void)audiodsp_tank_set_taps(config, taps, count * 4u);
...
void audiodsp_tank_configure(audiodsp_tank_config_t *config,
    audiodsp_tank_option_t option, float value) {
    switch (option) {
        case AUDIODSP_TANK_OPT_DECAY:
            ...
            break;
        case AUDIODSP_TANK_OPT_DIFFUSION:
        ...
        case AUDIODSP_TANK_OPT_TONE_DB:
            ...
            // with ln(10)/40
            // folded in rather than powf, which is how audiodsp_dynamics.c:17
            // does its own dB conversion and keeps powf out of an mcu build.
            config->tone_low_gain =
                expf(-config->tone_db * AUDIODSP_TANK_DB_TO_LN);
            config->tone_high_gain =
                expf(config->tone_db * AUDIODSP_TANK_DB_TO_LN);
            config->tone_coef = one_pole_coefficient(
                AUDIODSP_TANK_TONE_PIVOT_HZ, config->sample_rate);
            break;
        case AUDIODSP_TANK_OPT_MIX:
...
void audiodsp_tank_config_finish(audiodsp_tank_config_t *config) {
    audiodsp_tank_configure(config, AUDIODSP_TANK_OPT_DAMPING_HZ,
        config->damping_hz);
    audiodsp_tank_configure(config, AUDIODSP_TANK_OPT_BANDWIDTH_HZ,
        config->bandwidth_hz);
    audiodsp_tank_configure(config, AUDIODSP_TANK_OPT_LOW_CUT_HZ,
        config->low_cut_hz);
    audiodsp_tank_configure(config, AUDIODSP_TANK_OPT_MOD_RATE_HZ,
        config->mod_rate_hz);
    audiodsp_tank_configure(config, AUDIODSP_TANK_OPT_MOD_DEPTH_MS,
        config->mod_depth_ms);
    audiodsp_tank_configure(config, AUDIODSP_TANK_OPT_PREDELAY_MS,
        config->predelay_ms);
    audiodsp_tank_configure(config, AUDIODSP_TANK_OPT_TONE_DB, config->tone_db);
...
uint32_t audiodsp_tank_buffer_samples(const audiodsp_tank_config_t *config) {
...
void audiodsp_tank_state_init(audiodsp_tank_state_t *state,
    const audiodsp_tank_config_t *config, int16_t *buffer) {
...
void audiodsp_tank_reset(audiodsp_tank_state_t *state,
    const audiodsp_tank_config_t *config) {
    if (state->lines[0] != NULL) {
        memset(state->lines[0], 0,
            (size_t)audiodsp_tank_buffer_samples(config) * sizeof(int16_t));
    }
    int16_t *buffer = state->lines[0];
    audiodsp_tank_state_init(state, config, buffer);
}

// The one place the whole thing is not linear, and the same cubic
// audiodsp_feedback_delay.c soft-clips its loop with: odd-symmetric, so it
...
// one audiodsp_feedback_delay.c hands its blocks out with.
...
void audiodsp_tank_process_s16(const audiodsp_tank_config_t *config,
    ...
    const float g_input_2 = config->diffusion * AUDIODSP_TANK_INPUT_DIFFUSION_2;
    const float g_tank_1 = config->diffusion * AUDIODSP_TANK_DECAY_DIFFUSION_1;
    const float g_tank_2 = config->diffusion * AUDIODSP_TANK_DECAY_DIFFUSION_2;
