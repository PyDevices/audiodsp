...
static mp_obj_t audioecho_feedback_delay_sources(mp_obj_t self_in, uint8_t index) {
    return index == 0 ? ((audioecho_feedback_delay_obj_t *)MP_OBJ_TO_PTR(self_in))->source
                      : MP_OBJ_NULL;
}
...
static const audiosample_p_t audioecho_feedback_delay_proto = {
    ...
    .sources = audioecho_feedback_delay_sources,
};
