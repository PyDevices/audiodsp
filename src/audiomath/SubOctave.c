...
static mp_obj_t audiomath_suboctave_sources(mp_obj_t self_in, uint8_t index) {
    return index == 0 ? ((audiomath_suboctave_obj_t *)MP_OBJ_TO_PTR(self_in))->source
                      : MP_OBJ_NULL;
}
...
static const audiosample_p_t audiomath_suboctave_proto = {
    ...
    .sources = audiomath_suboctave_sources,
};
