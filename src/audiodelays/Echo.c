        ...
        if (self->sample == NULL) {
            ...
            if (mix <= MICROPY_FLOAT_CONST(0.01)) {
                ...
                if (self->base.bits_per_sample == 16 && self->base.samples_signed &&
                    ...
                    audiodsp_echo_positions_t positions = {
                        self->echo_buffer_left_pos, self->echo_buffer_right_pos};
                    audiodsp_echo_process_s16(word_buffer, sample_src, n,
...
static mp_obj_t audiodelays_echo_sources(mp_obj_t self_in, uint8_t index) {
    return index == 0 ? ((audiodelays_echo_obj_t *)MP_OBJ_TO_PTR(self_in))->sample
                      : MP_OBJ_NULL;
}
...
static const audiosample_p_t audiodelays_echo_proto = {
    ...
    .sources = audiodelays_echo_sources,
};
