    ...
    while (length != 0) {
        ...
        if (self->sample == NULL) {
            ...
            if (self->base.bits_per_sample == 16 && self->base.samples_signed &&
                ...
                self->chorus_buffer_pos = audiodsp_chorus_process_s16(
...
static mp_obj_t audiodelays_chorus_sources(mp_obj_t self_in, uint8_t index) {
    return index == 0 ? ((audiodelays_chorus_obj_t *)MP_OBJ_TO_PTR(self_in))->sample
                      : MP_OBJ_NULL;
}
...
static const audiosample_p_t audiodelays_chorus_proto = {
    ...
    .sources = audiodelays_chorus_sources,
};
