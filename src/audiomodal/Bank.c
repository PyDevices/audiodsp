...
static mp_obj_t audiomodal_bank_sources(mp_obj_t self_in, uint8_t index) {
    return index == 0 ? ((audiomodal_bank_obj_t *)MP_OBJ_TO_PTR(self_in))->source
                      : MP_OBJ_NULL;
}
...
static const audiosample_p_t audiomodal_bank_proto = {
    ...
    .sources = audiomodal_bank_sources,
};
