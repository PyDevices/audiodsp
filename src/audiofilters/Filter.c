...
static mp_obj_t audiofilters_filter_sources(mp_obj_t self_in, uint8_t index) {
    return index == 0 ? ((audiofilters_filter_obj_t *)MP_OBJ_TO_PTR(self_in))->sample
                      : MP_OBJ_NULL;
}
...
static const audiosample_p_t audiofilters_filter_proto = {
    ...
    .sources = audiofilters_filter_sources,
};
