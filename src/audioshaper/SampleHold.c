...
static mp_obj_t audioshaper_samplehold_sources(mp_obj_t self_in, uint8_t index) {
    return index == 0 ? ((audioshaper_samplehold_obj_t *)MP_OBJ_TO_PTR(self_in))->source
                      : MP_OBJ_NULL;
}
...
static const audiosample_p_t audioshaper_samplehold_proto = {
    ...
    .sources = audioshaper_samplehold_sources,
};
